...
{
	unsigned long util = min_t(unsigned long, SCHED_CAPACITY_SCALE, cpu_util_freq(cpu, walt_load));
				   ...
				   struct find_best_target_env *fbt_env)
{
	...
	*backup_cpu = -1;
	...
	if (((capacity_orig_of(prev_cpu) == capacity_orig_of(cpu)) ||
		asym_cap_siblings(prev_cpu, cpu)) &&
...
out:
	...
	trace_sched_task_util(p, next_cpu, backup_cpu, target_cpu, sync,
			fbt_env.need_idle, fbt_env.fastpath, placement_boost, start_t,
...
static int idle_balance(struct rq *this_rq, struct rq_flags *rf)
{
	...
	if (!(min_cap_cluster_has_misfit_task() && force_lb) && (avg_idle < sysctl_sched_migration_cost ||
	...
	for_each_domain(this_cpu, sd) {
		...
		if (!(min_cap_cluster_has_misfit_task() && force_lb) && avg_idle < curr_cost + sd->max_newidle_lb_cost) {
			update_next_balance(sd, &next_balance);
			break;
		}
